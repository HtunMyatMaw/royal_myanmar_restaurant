<template>
    <main class="container py-5">
        <h2 class="bottom_border mb-3 p-2">クイックメニュー</h2>
        <section>
            <!-- {{ getFriedFoods }} -->
            <div v-for="(food, index) in getFriedFoods" :class="index != getFriedFoods.length - 1 ? 'food-card-layout':''">
                <div v-if="index % 2 == 0"
                    class="d-flex flex-column d-lg-flex flex-lg-row justify-content-center">
                    ...
                    <div class="order-2 order-lg-2 align-self-center d-flex flex-column">
                        ...
                        <span class="bg-theme text-light py-1 px-4 mt-2 align-self-start rounded">{{ food.foodPrice
                            }}円</span>
                    </div>
                </div>
                ...
                <div v-else class="d-flex flex-column d-lg-flex flex-lg-row justify-content-center">
                    <div class="order-2 order-lg-1 align-self-center d-flex flex-column text-lg-end">
                        ...
                        <span class="bg-theme text-light py-1 px-4 mt-2 align-self-start align-self-lg-end rounded">{{ food.foodPrice
                            }}円</span>
                    </div>
...
export default {
    computed: {
        getFriedFoods() {
            return this.$store.getters.getFoods({ foodType: 5 })
        }
    },
    created() {
        this.$store.dispatch('getFoodAction', { foodType: 5 })
    }
}
</script>
